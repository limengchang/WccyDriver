...
import com.bfbyxx.wccydriver.entity.CityAddressBean;
...
public class AddressPickerView extends RelativeLayout implements View.OnClickListener {
    ...
    private List<CityAddressBean.AddressItemBean> mRvData; // 用来在recyclerview显示的数据
    private AddressAdapter mAdapter;   // recyclerview 的 adapter

    private CityAddressBean mYwpAddressBean; // 总数据
    private CityAddressBean.AddressItemBean mSelectProvice; //选中 省份 bean
    private CityAddressBean.AddressItemBean mSelectCity;//选中 城市  bean
    private CityAddressBean.AddressItemBean mSelectDistrict;//选中 区县  bean
    ...
    private OnAddressPickerCancelListener mOnAddressPickerCancelListener;
    ...
    private void initData() {
        ...
        // 将数据转换为对象
        mYwpAddressBean = new Gson().fromJson(jsonSB.toString(), CityAddressBean.class);
     ...
     */
    public void initData(CityAddressBean bean) {
        if (bean != null) {
    ...
    public void onClick(View v) {
        ...
        if (i == R.id.tvSure) {
            ...
            mOnAddressPickerCancelListener.onSureClick();
        }
    }
    ...
    private void sure() {
        if (mSelectProvice != null /*&&
            ...
            if (mOnAddressPickerSureListener != null) {
                ...
                String addressPri = mSelectProvice != null?mSelectProvice.getN():"";
                String addressCity = mSelectCity != null?mSelectCity.getN():"";
                String addressDistrict = mSelectDistrict != null?mSelectDistrict.getN():"";
                String address = addressPri+addressCity+addressDistrict;
                String strProviceCode = mSelectProvice != null?mSelectProvice.getI():"";
                String strCityCode = mSelectCity != null?mSelectCity.getI():"";
                String strDistrictCode = mSelectDistrict != null?mSelectDistrict.getI():"";
                mOnAddressPickerSureListener.onSureClick(address,strProviceCode,strCityCode,strDistrictCode);
            }
        } else {
            Toast.makeText(mContext, "请先选中地址再确定", Toast.LENGTH_SHORT).show();
        }
        ...
        public void onTabSelected(TabLayout.Tab tab) {
            ...
            switch (tab.getPosition()) {
                ...
                case 1:
                    // 点到城市的时候要判断有没有选择省份
                    if (mSelectProvice != null) {
                        for (CityAddressBean.AddressItemBean itemBean : mYwpAddressBean.getCity()) {
                ...
                case 2:
                    ...
                    if (mSelectProvice != null && mSelectCity != null) {
                        for (CityAddressBean.AddressItemBean itemBean : mYwpAddressBean.getDistrict()) {
...
//                            mTvSure.setTextColor(defaultSureUnClickColor);
     ...
     * 点取消回调这个接口
     */
    public interface OnAddressPickerCancelListener {
        void onSureClick();
    }

    public void setOnAddressPickerCancel(OnAddressPickerCancelListener listener) {
        this.mOnAddressPickerCancelListener = listener;
    }
