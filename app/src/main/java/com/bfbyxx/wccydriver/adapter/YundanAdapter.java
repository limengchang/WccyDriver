 ...
 * Time: 11:27
 */
public class YundanAdapter extends BaseAdapter {
    private List<Order> mList;//数据源
    private LayoutInflater mInflater;//布局装载器对象

    // 通过构造方法将数据源与数据适配器关联起来
    // context:要使用当前的Adapter的界面对象
    public YundanAdapter(Context context, List<Order> list) {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        // 设置控件的数据
        viewHolder.tv_order_status.setText(Tools.waybillStatus(mList.get(position).getWaybillStatus()));
        viewHolder.tv_order_no.setText("运单编号："+ mList.get(position).getWaybillNo());
